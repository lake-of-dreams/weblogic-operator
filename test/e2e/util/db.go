...
func (dbh *MySQLDBTestHelper) CreateDBTable(db string, table string, column string) {
	_, e := dbh.ex.ExecuteSQLForDB(db, "create table "+table+" ("+column+" varchar(256));")
	if e != nil {
...
func GetMySQLPassword(t *testing.T, podname string) string {
	...
	return strings.SplitN(output, "=", 2)[1]
}
