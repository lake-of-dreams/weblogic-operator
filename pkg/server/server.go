...
// GetStatefulSetForServer finds the associated StatefulSet for a Weblogic server
func GetStatefulSetForServer(server *types.WeblogicServer, kubeClient kubernetes.Interface) (*v1beta1.StatefulSet, error) {
...
func updateServer(server *types.WeblogicServer, restClient *rest.RESTClient) error {
...
func createServer(server *types.WeblogicServer, kubeClient kubernetes.Interface, restClient *rest.RESTClient) error {
	...
	if !HasServerNameLabel(server.Labels, server.Name) {
		...
		return updateServer(server, restClient)
	}
	...
	_, err = CreateStatefulSet(kubeClient, server, serverService)
	if err != nil {
		return err
	}
...
// When delete server is called we will delete the stateful set (which also deletes the associated service) and
// delete any secrets associated with the server
func deleteServer(server *types.WeblogicServer, kubeClient kubernetes.Interface, restClient *rest.RESTClient) error {
	err := server.Validate()
	if err != nil {
		return err
	}

	err = DeleteStatefulSet(kubeClient, server)
	if err != nil {
		return err
	}

	err = DeleteService(kubeClient, server)
	if err != nil {
		return err
	}
...
func updateServerWithStatefulSet(server *types.WeblogicServer,
...
func CreateStatefulSet(clientset kubernetes.Interface, server *types.WeblogicServer, service *v1.Service) (*v1beta1.StatefulSet, error) {
	// Find StatefulSet and if it does not exist create it
	existingStatefulSet, err := GetStatefulSetForServer(server, clientset)
	if err != nil {
...
func GetServerForStatefulSet(statefulSet *v1beta1.StatefulSet, restClient *rest.RESTClient) (server *types.WeblogicServer, err error) {
	if mySQLServerName, ok := statefulSet.Labels[constants.WeblogicServerLabel]; ok {
...
func DeleteStatefulSet(clientset kubernetes.Interface, server *types.WeblogicServer) error {
	statefulSet, err := GetStatefulSetForServer(server, clientset)
...
// DeleteService deletes the Service associated with a Weblogic server.
func DeleteService(clientset kubernetes.Interface, server *types.WeblogicServer) error {
